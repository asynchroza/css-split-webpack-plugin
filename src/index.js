...
export default class CSSSplitWebpackPlugin {
     ...
     */
    constructor({ size = 4000, imports = false, filename = '[name]-[part].[ext]', preserve, defer = false }) {
        this.options = {
            size,
    ...
    file(key, asset) {
        ...
        const input = asset.sourceAndMap
            ? asset.sourceAndMap()
            : {
                  source: asset.source(),
              };
        ...
        return postcss([chunk(this.options)])
            ...
            .then((result) => {
                return Promise.resolve({
                    ...
                    chunks: result.chunks.map(({ css, map }, i) => {
                        ...
                        const result = map ? new SourceMapSource(css, name, map.toString()) : new RawSource(css);
                        result.name = name;
                        return result;
                    }),
                });
        ...
        const promises = Array.from(chunks).map((chunk) => {
            ...
            return Promise.all(items).then((entries) => {
                entries.forEach((entry) => {
                    ...
                    const content = Array.from(entry.chunks)
                        .map((file) => {
    ...
    apply(compiler) {
        if (this.options.defer) {
            ...
            compiler.hooks.thisCompilation.tap(name, (compilation) => {
                ...
                compilation.hooks.optimizeChunkAssets.tapAsync(name, (chunks, done) => {
